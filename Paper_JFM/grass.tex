...
Its instability, through a mechanism similar to the Kelvin-Helmholtz instability, is thought to lead to coherent eddies over the grass bed.
The grass responds to these eddies by deforming, which manifests itself as large amplitude synchronous oscillations.
Basic application of this model have been used to predict the observed frequency of \monami~and to understand transport in the seagrass bed \cite{Nepf00,Ghisal02,Nepf04,Okamoto12}.
...
We find that Mode 1 resembles many characteristics of Kelvin-Helmholtz instability but with the inclusion of the vegetation drag. 
Mode 2 is not related to the Kelvin-Helmholtz instability mechanism and arises purely due to the interaction of an inviscid layer of fluid with submerged rigid.
...
Based on this analysis, we conclude that Mode 2 is distinct from the Kelvin-Helmholtz instability, and owes its existence to the vegetation drag.
...
Experimental observations show that the boundary layer thickness scales instead as $\Ndg^{-1}$.
...
We have captured one realization of this feature using constant eddy viscosity.
Experiments have also shown that a mixing length $l$ model better approximates the turbulent characteristics of the flow with $l = \delta$; the boundary layer itself establishes eddies to transport momentum. 
...
The driving mechanism for this instability is similar to that of Kelvin-Helmholtz, and relies only on the presence of this shear as presented in the $U_{\eta\eta}$ term in equation \eqref{eqn:mode1asymp}. 
Therefore, we expect Mode 1 instability to be exhibited independent of the turbulence model. 
We further expect the fastest growing wavenumber to be proportional to $1/\delta$, and the mode to be localized to the boundary layer.
