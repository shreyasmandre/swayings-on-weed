...
Basic application of this model have been used to predict the observed frequency of \monami~and to understand transport in the seagrass bed \citep{Nepf00,Ghisal02,Nepf04,Okamoto12}.

However, several aspects of this explanation remain unsatisfactory. 
The instability is modeled using the inviscid Rayleigh equation and the role of drag in damping the coherent perturbations to the shear profile is ignored \citep{Raupach96}. 
It is so despite the shear layer itself being produced as a result of the vegetation drag.
Furthermore, the shear layer is assumed to be not influenced by the top and bottom boundary of the domain, although the experimentally measured thickness of the shear layer is in many cases comparable to the unvegetated water depth.
...
While these profiles qualitatively capture the features of the profile, the dependence of the shear layer thickness on the parameters of the problem remains unexplained.
...
Here we present a mathematical model for the linear instability that accounts correctly for these effects.
The vegetation is modeled as a continuum through a drag on the fluid proportional to the grass blade density. 
...
The presence of vegetation drag leads to a finite threshold flow condition for onset of the instability for both these modes.

\section{Mathematical model}
Although \monami ~is manifested in the motion of the grass, the drag exerted by the grass bed on the flow is central to the hypothesized instability. 
...
\end{center}
\caption{
Critical Reynolds number (left) and the corresponding marginally stable wave number (right) for different submergence ratio as a function of vegetation density parametrized by the boundary layer thickness. 
Parameters from experiments reported in ref. \cite{Ghisal02} to exhibit or suppress synchronous waving are also included in the left panel. 
}
\label{Re_vs_delta}
\end{figure}
...
The dependence of this threshold $\Rey$, and the corresponding marginally stable wavenumber $k$, on $\delta/H$ and $\hg/H$ is shown in Fig.~\ref{Re_vs_delta}, and is found to compare well with experimental observations \citep{Ghisal02}.
...
Comparison of this predicted threshold against estimated Reynolds number for lab scale experiments are shown in  Fig \ref{Re_vs_delta}. We have also compared frequency (Im$(\sigma)$) of the fastest growing mode with the observed frequencies -- frequency of \monami, maxima in the velocity spectra, and frequency of vortex passage in lab scale experiments \citep{Ghisal02} -- for cases where the vegetation was sufficiently dense to be modeled by a continuum drag field, as shown in Fig.~\ref{frequency_comparison}.  
...
The distinct asymptotic behavior of the two modes as $\Ndg \gg 1$ distinguishes it from the KH instability. 
\subsection{Mode 1}
...
To further understand the mechanism of Mode 1, we rescale \eqref{Orr-somerfield} in the the boundary layer using $\eta = y/(\delta/H)$, 
...
Therefore, based on the inviscid and viscous formulations of KH instability, the origin of the threshold flow conditions observed in experiments and the field is unclear.
...
Based on this analysis, we conclude that Mode 2 is distinct from the KH instability, and owes its existence to the vegetation drag.
...
The simplicity of this model allows us to make progress and capture the essential features of the instability without accounting for the rich and detailed characteristics of the turbulence. 
...
As a consequence of the constant eddy viscosity, the boundary layer thickness $\delta$ scales as $\ReyNdg^{-1/3}$.
...
Experiments have also shown that a model based on mixing length $l$ better approximates the turbulent characteristics of the flow with $l = \delta$; \textit{i.e.}, the boundary layer itself establishes eddies to transport momentum. 
The eddy viscosity corresponding to this model is $\mu = \rho U \delta$, and the leading order balance between turbulent momentum transport and vegetation drag is $\mu U/\delta^2 = \rho C_N d N_g U^2$.
Substituting $\mu$ yields $\delta/H = \Ndg^{-1}$, in agreement with the experimental observations.
Within our framework, the mixing length model implies a scale for the eddy viscosity $\mu = \rho U_g \delta$ at grass top, which corresponds to an effective $\Rey = U_0H/U_g \delta$.
Furthermore, matching the slope of the velocity profile from the boundary layer to the unvegetated flow implied $U_g/U_0 \sim \delta/H$, and therefore $\Rey = (H/\delta)^2$.
Substituting this relation in $\delta/H = (\Rey \Ndg)^{-1/3}$ and solving for $\delta$ yields $\delta/H = \Ndg^{-1}$, which is in agreement with experiments and the simple scaling analysis.
...
In real meadows, the drag coefficients are known to vary from bottom to tip of the grass blades \citep{Vivoni98,Nepf00}. 
