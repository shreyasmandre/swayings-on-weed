...
The vegetation drag on the flow is approximated by a continuous body force $\mathbf{f}=-N_g\mathbf{f_d}$,  where $\mathbf{f_{d}}$ is the drag force per unit length of the grass blade, and $N_g$ the grass blade number density per unit horizontal area \cite{Vivoni98,Nepf99,Ghisal02,Delangre04,Delangre06}. 
...
The Reynolds number of the flow based on the scale of the grass blade is $O(10^2-10^3)$; therefore, neglecting the skin friction, we model it based on the form drag on the vegetation, $\mathbf{f_{d}}=C_N \rho u^{2}d\bx$, where $d$ is the average width of grass blade projected in the flow direction, $C_{N}$ the drag coefficient. 
% We model it as 
...
The former arises because, in the case of dense vegetation, the shear stress exerted by the bottom surface is expected to be negligible compared to the vegetation drag, whereas the latter is due to the free interface. 
...
Because this boundary layer develops from purely local dynamics, independent of the influence of the boundaries, we identify it to be analogous to the free shear layer \cite{Ghisal02} in the previous explanation of \monami.
