...
\begin{document}
\title{The onset of hydrodynamically driven waving of marine vegetation in flow}
\author{Ravi Singh}
...
% \end{abstract}
\begin{abstract}
We develop a self-consistent mathematical model for the onset of synchronous waving of dense marine vegetation driven by a hydrodynamic instability.
The vegetation, assumed rigid, only exerts a drag force on the flow.
We show that a linear instability of a steady pressure-driven flow explains the experimentally observed flow threshold for waving and the waving frequency,
 confirming the hydrodynamic mechanism for the onset of waving, where the vegetation flexibility merely serves to visualize the flow.
We demonstrate that flow instability mechanism leading to waving is different from the Kelvin-Helmholtz instability.
\end{abstract}
% Asymptotic analysis of the model for dense grass reveals two unstable modes. 
% We show this by  
%  naturally leads to the threshold condition, and explains its mechanistic origin. 
\maketitle
...
$\mathbf{f_{d}}=C_N \rho u_{N}^{2}d\bn$, \shreyas{citation needed} 
...
\begin{figure*}
\begin{tabular}{cccc}
{\includegraphics[scale = .95]{Set4_dens28_imgsc}} &
%{\includegraphics[scale = 0.78]{Set4_dens30_imgsc}} &
{\includegraphics[scale = .95]{Set4_dens32_imgsc}} &
{\includegraphics[scale = .95]{Set4_dens34_imgsc}}\\
{\includegraphics[scale = .95]{Set5_dens38_imgsc}} &
%{\includegraphics[scale = 0.78]{Set5_dens40_imgsc}} &
{\includegraphics[scale = .95]{Set5_dens42_imgsc}} &
{\includegraphics[scale = .95]{Set5_dens46_imgsc}} \\
...
\acknowledgments
MMB was hosted by Brown University during this work and was supported by the OIST Graduate University with subsidy funding from the Cabinet Office, Japan. We thanks Heidi Nepf and Marco Ghisalberti for helpful discussions.
