...
Although it predicts the \monami ~waving frequency, several of its aspects remain unsatisfactory: 
(i) The profile of the shear layer is assumed \textit{ad hoc} to be piecewise linear \cite{Delangre06} or hyperbolic tangent \cite{Ghisal02,Raupach96}. 
    The role of drag in damping the perturbations to the shear profile is also sometimes ignored \cite{Raupach96}. 
...
(ii) Observations in the lab \cite{Ghisal02} and the field \cite{Grizzle96} indicate the existence of a threshold flow speed below which \monami ~is not observed. No existing theory has explained this threshold condition. 
...
The instability and the resulting flow structures persist even if the grass blades are replaced by rigid dowels \cite{Ghisal02}. 
...
The criteria for the validity of this assumption is verified \textit{a posteriori} in this letter.
...
The drag exerted by the vegetation on the flow is approximated via a continuous body force $\mathbf{f}=-N_g\mathbf{f_d}$,  
...
% =======
% {\includegraphics[scale = 0.93]{Set4_dens28_imgsc}} &
% %{\includegraphics[scale = 0.67]{Set4_dens30_imgsc}} &
% {\includegraphics[scale = 0.93]{Set4_dens32_imgsc}} &
% {\includegraphics[scale = 0.93]{Set4_dens34_imgsc}} \\
% {\includegraphics[scale = 0.93]{Set5_dens38_imgsc}} &
% %{\includegraphics[scale = 0.67]{Set5_dens40_imgsc}} &
% {\includegraphics[scale = 0.93]{Set5_dens42_imgsc}} &
% {\includegraphics[scale = 0.93]{Set5_dens46_imgsc}} \\
% >>>>>>> origin/master
...
This consideration can be cast using scaling analysis into a simple criteria that the buoyancy time scale  be much faster than the hydrodynamic time scale $H/U_0$.
...
Using representative values for a common sea grass, \textit{Zostera Marina}, $\Delta \rho /\rho \approx 0.25$, $V_f \approx 0.1$ and $H=1$ m \cite{Fonseca98} yields the time buoyancy time scale to be about 2 s. 
