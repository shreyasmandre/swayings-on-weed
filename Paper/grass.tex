...
\affiliation{Woods Hole Oceanographic Institute, Woods Hole MA USA}
...
$P$ the pressure, $\mu$ the eddy viscosity (the flow is usually turbulent) and $\mathbf{g}$ the acceleration due to gravity. 
...
We solve this eigenvalue problem and a threshold in $\Rey$ of $O(100)$, above which the flow is unstable (real part of $\sigma$ is positive), emerges from the 
