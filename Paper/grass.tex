...
We study the mechanism behind synchronous waving of marine grass, known as Mo-nami. We do linear stability analysis of a mathematical model which considers grass to exert a continuum drag on the flow.
...
which compares well with experimental observations. Our analysis further predicts two distinct mode of instability which might lead to Mo-nami.
One of these mode shares many characteristics with the Kelvin-Helmholtz instability, we termed this mode as ``Mode 1''. The other mode can be thought of as instability of 
inviscid flow above grass coupled with the porous flow through grass bed, we refer to this mode as ``Mode 2''.  In the limit of dense grass bed these two modes can be asymptotically 
distinguished. In the parameter regime typical of marine grass beds, the two modes merge and are indistinguishable.
