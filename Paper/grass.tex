...
This model has been applied successfully to predict the the observed frequency of \monami ...!
\newline
...
Therefore, to develop the essential mathematical model, we assume the grass blades to be rigid, on average oriented along the $y$ direction, and verify the assumption \textit{a posteriori}.
We show that this drag results in an instability mode different from the Kelvin-Helmholtz instability.
\begin{figure}[b]
...
Inset compares experimental observations of the experimentally measured dominant frequency $f_o$ (in Hz) with the predictions $f_p$ from the solution of eq.~\eqref{Orr-somerfield}. 
...
The Reynolds number of the flow based on the scale of the grass blade is $O(10^2-10^3)$; therefore, neglecting the skin friction, we model it based on the form drag on the vegetation, $\mathbf{f}=N_g C_N \rho u^{2}d\bx$, where  $N_g$ is the blade number density per unit horizontal area, $C_{N}$ the drag coefficient, and $d$ the average blade width projected perpendicular to the flow. 
% We model it as 
...
Based on previous experiments \cite{Vivoni98,Nepf00}, we take $C_N \approx 1$.
\newline
\begin{figure*}
 \includegraphics{SetAll_imgsc_hsv}
%\end{figure}
%\begin{figure*}
%\begin{tabular}{cccc}
...
This expectation is verified by a comparison, shown in fig.~\ref{basicflow}, of the steady flow profile from the solution of eq.~\eqref{base_equ} with experimental measurements.
...
\newline
Next we substitute $\bu = (U+\tilde{u}, \tilde{v})$, $p=P+\tilde{p}$ in eq.~\eqref{navier-stokes} and expand to linear order to investigate the evolution of small perturbations $(\tilde{u}, \tilde{v})$. These perturbations obey (dropping the tilde decoration)
\begin{equation}
\begin{split}
...
The growth rate $\sigma$ for a given wave number $k$ appears as an eigenvalue that allows a non-trivial solution $\phi$ of eq. \eqref{Orr-somerfield}.

A threshold in $\Rey$, above which the flow is unstable (Re$(\sigma)>0$) for at least one $k$, emerges from the 
solution of eq.~\eqref{Orr-somerfield}. The dependence of this threshold $\Rey$, and the corresponding marginally stable wavenumber $k$, on $\delta/H$ and $\hg/H$ is shown in fig.~\ref{Re_vs_delta}, and is found to compare well with experimental observations \cite{Ghisal02}.
...
In this limit eq.~\eqref{Orr-somerfield} simplifies to 
\begin{align}
\begin{split}
