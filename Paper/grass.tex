...
We study the mechanism behind Mo-nami, a phenomenon associated with the synchronous waving of marine grass. Flow through grass bed gives rise to a region of strong shear near tip of grass 
known as shear layer, due to vertical discontinuity of grass drag. Linear stability analysis of a mathematical model incorporating grass drag as a continuum field has been 
...
which compares well with experimental observations. Further analysis predicts existence of two distinct mode of instability.
One of these mode has many characteristics similar to the Kelvin-Helmholtz instability where shear layer plays a central role, we termed this as ``Mode 1''. 
The other mode can be interpreted as instability of 
inviscid flow through a grass bed where shear layer doesn't play a critical role, we refer this as ``Mode 2''. In the limit of dense grass bed these two modes can be
distinguished. For the parameter regime typical of marine grass beds, the two modes merge and are indistinguishable.
